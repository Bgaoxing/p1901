...
class BeikekeSpider(scrapy.Spider):
    ...
    data_url = 'https://cd.fang.ke.com/loupan/{}/pg{}/#{}'  # 链接
    ...
    def parse(self, response):
        ...
        bs = bs4.BeautifulSoup(response.text)  # 解析html
        numbers = bs.find_all('section', attrs={'class': "se-part"})[-1]
        number = numbers.select('a')[-1].text  # 总共的页码
        ...
        for item_li in items_div.find_all('li', attrs={'class': 'resblock-list post_ulog_exposure_scroll has-results'},
                                          recursive=True):
        ...
        # 开始爬取多页
        self.y += 1
        if self.y <= int(number):  # 可以看到贝壳房-新房，网站上的页码
